...
public class CambioCamara : MonoBehaviour {
	...
	void Update () {
		...
		if (!firstPerson && Input.GetKey(KeyCode.W) && overCamera.transform.position.z < 0) {
			overCamera.transform.Translate(0,0.5f,0);
		}
		if (!firstPerson && Input.GetKey(KeyCode.S) && overCamera.transform.position.z > -8.5) {
			overCamera.transform.Translate(0,-0.5f,0);
		}
	}
}
